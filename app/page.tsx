      ...
      <section className="bg-gray-50 py-16">
        <div className="max-w-5xl mx-auto px-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 max-w-3xl mx-auto">
            {(isAr ? [
              { name: 'أساسية', price: '349', period: 'شهرياً', features: ['٤ حصص شهرياً', 'متابعة واتساب', 'دعم 24/7'], popular: false },
              { name: 'متقدمة', price: '599', period: 'شهرياً', features: ['٨ حصص شهرياً', 'متابعة يومية', 'تمارين مخصصة', 'دعم 24/7'], popular: true },
              { name: 'احترافية', price: '999', period: 'شهرياً', features: ['١٦ حصة شهرياً', 'متابعة يومية', 'تمارين مخصصة', 'تحضير مقابلات', 'دعم 24/7'], popular: false },
            ] : [
              { name: 'Basic', price: '349', period: '/mo', features: ['4 sessions/month', 'WhatsApp follow-up', '24/7 support'], popular: false },
              { name: 'Advanced', price: '599', period: '/mo', features: ['8 sessions/month', 'Daily follow-up', 'Custom exercises', '24/7 support'], popular: true },
              { name: 'Pro', price: '999', period: '/mo', features: ['16 sessions/month', 'Daily follow-up', 'Custom exercises', 'Interview prep', '24/7 support'], popular: false },
            ]).map((p, i) => (
              <div key={i} className={`rounded-2xl p-6 flex flex-col ${p.popular ? 'bg-[#10B981] text-white shadow-lg shadow-emerald-200 scale-105' : 'bg-white border border-gray-100'}`}>
                {p.popular && (
                  <div className="text-xs font-bold bg-white/20 text-white px-3 py-1 rounded-full w-fit mb-3">
                    {isAr ? '⭐ الأكثر طلباً' : '⭐ Most Popular'}
                  </div>
                )}
                <div className={`font-bold text-lg mb-1 ${p.popular ? 'text-white' : 'text-gray-900'}`}>{p.name}</div>
                <div className={`text-3xl font-black mb-1 ${p.popular ? 'text-white' : 'text-[#10B981]'}`}>
                  {p.price} <span className={`text-sm font-normal ${p.popular ? 'text-white/80' : 'text-gray-400'}`}>{isAr ? 'جنيه' : 'EGP'} {p.period}</span>
                </div>
                <ul className="mt-4 mb-6 flex-1 space-y-2">
                  {p.features.map((f, j) => (
                    <li key={j} className={`text-sm flex items-center gap-2 ${p.popular ? 'text-white/90' : 'text-gray-600'}`}>
                      <span className={p.popular ? 'text-white' : 'text-[#10B981]'}>✓</span> {f}
                    </li>
                  ))}
                </ul>
                <Link
                  ...
                  className={`block text-center py-2.5 rounded-xl text-sm font-bold transition ${
                    p.popular ? 'bg-white text-[#10B981] hover:bg-gray-100' : 'bg-[#10B981] text-white hover:bg-emerald-600'
                  }`}
                >
